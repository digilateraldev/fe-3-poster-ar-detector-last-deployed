...
const BMISelectionApp = () => {
  ...
  const handleZoneDetection = (zoneName) => {
    ...
    if (zoneName !== currentZone) {
      setCurrentZone(zoneName);
      setPointerStartTime(zoneName ? Date.now() : null);
      setSelectionReady(false);
      return;
    }

    // Check duration
    if (zoneName && pointerStartTime) {
      const duration = (Date.now() - pointerStartTime) / 1000;
      if (duration >= 2 && !selectionReady) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      window.location.href = `/play?zone=${currentZone}`;
    } catch (err) {
