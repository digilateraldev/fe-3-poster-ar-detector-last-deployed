...
function detectFingerFromContoursJS(src) {
  let hsv = new cv.Mat();
  let mask = new cv.Mat();
  ...
  let lower = new cv.Mat(hsv.rows, hsv.cols, hsv.type(), [0, 48, 80, 0]);
  ...
  let point = null;
  if (bestContour) {
    let topmost = bestContour.data32S;
    let minY = Infinity;
  ...
  hsv.delete(); mask.delete(); kernel.delete();
...
const cornerZones = {
  ...
  // Fallback to OpenCV.js contour detection
  const canvas = canvasRef.current;
  const ctx = canvas.getContext("2d");
  const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
  let src = cv.matFromImageData(imageData);

  const fingertip = detectFingerFromContoursJS(src);
  src.delete();
  ...
  if (fingertip) {
    ...
    setWarningMessage("👆 No hand or fingertip visible.");
  }
}
