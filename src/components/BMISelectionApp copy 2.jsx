...
const BMISelectionApp = () => {
  ...
  const [pointerStartTime, setPointerStartTime] = useState(null);
  const [qrId, setQrId] = useState(null);
  const [showToast, setShowToast] = useState(false);
  const [detectedRegion, setDetectedRegion] = useState(null);
  const [videoResolution, setVideoResolution] = useState({ width: 1517, height: 2200 });
...
const scaleZone = (zone) =>
  zone.map(([x, y]) => [
    (x / 1517) * videoResolution.width,
    (y / 2200) * videoResolution.height,
  ]);
  ...
  const detectFingertipFromContours = (imageData, width, height) => {
    try {
      ...
      return {
        x: (topmost.x / width) * videoResolution.width
,
        y: (topmost.y / height) * videoResolution.height,
      };
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        qrId,
        selection: currentZone,
        deviceId, // deviceId will be auto-added by apiUtils.post
      };

      await apiUtils.post("/selection/store", payload);

      // Navigate to video player page
      window.location.href = `/selection/result?zone=${currentZone}`;
    } catch (err) {
  ...
  useEffect(() => {
    if (showToast && detectedRegion) {
      const regionTitle = zoneInfo[detectedRegion]?.title || "Unknown Region";
      alert(`Region detected:\n${regionTitle} (key: ${detectedRegion})`);
    }
  }, [showToast, detectedRegion]);
  ...
  const scaledZone = scaleZone(zone);
  if (isPointInZone([x, y], scaledZone)) {
    detected = zoneName;
    break;
  }
}
  ...
  // Set actual video resolution dynamically
  ...
  detectionCanvas.height = height;
      ...
      hands.onResults((results) => {
        ...
        if (
          ...
          // };
          fingerTip = {
  x: indexTip.x * videoResolution.width,
  y: indexTip.y * videoResolution.height,
};
        ...
        if (fingerTip) {
          ...
          if (detected) {
            handleZoneDetection(detected); // only call when something is pointed
          } else {
            setCurrentZone(null);
          }
      ...
      const scaleContainer = () => {
        ...
        const scaleX = wrapper.clientWidth / videoResolution.width;
        const scaleY = wrapper.clientHeight / videoResolution.height;
      ...
      <h1 className="text-2xl font-bold text-[#046a81]">Scanner</h1>
        ...
        >
          <video
            ...
            autoPlay
            playsInline
            className="w-full h-full object-cover bg-opacity-20 bg-black"
          />
          ...
          <div
            ref={pointerRef}
            className="absolute rounded-full border-2 border-white pointer-events-none hidden"
            style={{
              width: "30px",
              height: "30px",
              background:
          ...
          <div
            ref={resultRef}
            className="absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-70 text-white px-4 py-3 rounded-lg text-lg font-sans"
          >
            Loading...
          </div>

          {warningMessage && (
            <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 bg-orange-200 text-black px-3 py-2 rounded-lg font-sans font-bold">
              ⚠️ {warningMessage}
            </div>
          )}

          <div className="absolute top-5 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-70 text-white px-3 py-2 rounded text-center font-sans">
        ...
        {showToast && detectedRegion && zoneInfo[detectedRegion] && (
          <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 bg-white p-5 rounded-lg shadow-lg z-50 text-center">
            <p className="text-lg mb-3 text-black">
              You selected:{" "}
              <strong>
                {zoneInfo[detectedRegion]?.title ||
                  detectedRegion ||
                  "Unknown region"}
              </strong>
            </p>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    window.location.href = `/selection/result?qrId=${qrId}&region=${detectedRegion}`;
                  } catch (err) {
...
//               Detected: {zoneInfo[currentZone].title}
//             </h3>
//             <div>
//               <button
